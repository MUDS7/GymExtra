...
> 测试期间，小程序启动时会弹出已注册人员列表；首次调用会自动创建“测试用户小李”和“测试用户小王”。选择后会以该人员的训练、群组和统计数据进入。此功能通过请求中的 `testUserId` 实现，仅用于测试，上线前请移除相关前端弹窗与云函数 `identity.js` 的测试身份逻辑。

动作数据存放在云数据库的 `actions` 集合。部署最新版 `quickstartFunctions`
云函数后，首次进入动作选择页会自动创建集合并写入动作表。动作图片优先使用
随小程序发布的本地线条示意图；本地没有对应图片时，才读取云端记录中的文件 ID，
下载并保存到小程序本地文件系统，后续直接复用该缓存。本地动作表仅作为云端不可用时的兜底数据。
